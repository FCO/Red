...
Receives a hash of `AST`s of code and returns a `ResultSeq` using that `AST`s as filter Usually passed unique values as IDs or columns with unique counstraint
