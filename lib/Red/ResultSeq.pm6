...
sub create-comment-to-caller {
    my Str $meth-name = callframe(1).code.name;
    do given callframe(3) {
        my Str $msg = qq:to/END/;
            method '$meth-name' called at:
            file : { .file }
            block: { .code.name }
            line : { .line }
            END
        Red::AST::Comment.new: :$msg
    }
}
...
method Seq {
    @!comments.push: create-comment-to-caller;
